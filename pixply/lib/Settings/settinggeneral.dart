...
import 'package:pixply/Settings/blutoothcheck.dart';
// import 'package:pixply/Settings/design_game_page.dart';
...
import 'package:pixply/Settings/help.dart';
...
import 'package:pixply/Settings/aboutus.dart';
import 'dart:math' as math;
class SettingsGeneral extends StatefulWidget {
  final LedBluetooth bluetooth;
  final bool isConnected;

  const SettingsGeneral({super.key, required this.bluetooth, required this.isConnected});
...
void toggleSection(String section) {
  ...
  Future<void> setBrightness(double value) async {
    ...
    // بروزرسانی مقدار روشنایی در ColorConfig تا DisplayManager از آن استفاده کند
...
// }
  ...
  Widget _buildTile(String label, VoidCallback onTap, {
  String? iconPath,
  double iconWidth = 36,
  double iconHeight = 36,
  Widget? trailing,
  }) {
    final bool isLanguageTile = label == "Language";
    ...
    return GestureDetector(
      ...
      margin: const EdgeInsets.only(top: 8),
        ...
        child: Row(
          ...
          height: iconHeight,
              colorFilter: () {
                // اگر تایتل Language باشه → شفاف کن
                if (isLanguageTile) {
                ...
                // اگر آیکون از نوع bluetooth باشه → منطق سبز/سفید قبلی
                if (iconPath.contains('bluetooth')) {
                  return ColorFilter.mode(
                    widget.isConnected ? Colors.green : Colors.white,
                    BlendMode.srcIn,
                  );
                }
                // در غیر این صورت، آیکون رنگی خودش بمونه
                return null;
              }(),
            ),
            ...
            const SizedBox(width: 12),
            Text(label,
                style: TextStyle(color: isLanguageTile
                  ? Colors.white.withValues(alpha: 0.2)
                  : Colors.white, fontFamily: 'Poppins', fontSize: 24, fontWeight: FontWeight.w600)),
            const Spacer(),
            trailing ??
...
//       context,
//       MaterialPageRoute(
//         builder: (context) =>  UserPage (bluetooth: widget.bluetooth, isConnected: widget.isConnected),
//       ),
//     );
//   },
//   iconPath: 'assets/account.svg',
//   iconWidth: 27,
//   iconHeight: 33,
// ),

_buildTile(
  widget.isConnected ? "Connected" : "No Connected Device",
  () {
    Navigator.push(
      context,
      MaterialPageRoute(builder: (context) =>  BluetoothCheckPage( bluetooth: widget.bluetooth , isConnected: widget.isConnected,) ),
    );
  },
              iconPath: 'assets/bluetooth.svg',
              iconWidth: 38,
              iconHeight: 38,
              trailing: null,
            ...
            _buildTile("Brightness", () => toggleSection("brightness"), iconPath: 'assets/brightness.svg' ,iconWidth: 38,
              iconHeight: 38,),
...
_buildTile(
  ...
  iconPath: 'assets/doubt.svg',
    ...
    Navigator.push(
      context,
      MaterialPageRoute(builder: (context) => YourIdeaPage()),
    );
  },
              iconPath: 'assets/business.svg',
              iconHeight: 37,
              iconWidth: 37,
  ...
  () {
      Navigator.push(
      context,
      MaterialPageRoute(builder: (context) => AboutUsPage( bluetooth: widget.bluetooth, isConnected: widget.isConnected,) ),
    );
  },
...
//       Navigator.push(
//       context,
//       MaterialPageRoute(builder: (context) => BrakeConditionPage()),
//     );
//   },
